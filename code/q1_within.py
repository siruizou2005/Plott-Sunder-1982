...
Two convergence notions, kept strictly apart:
  ...
  CROSS-period   -- across the session, does the k-th repetition of a side / of a
                    ...
                    the PERIOD.  This is what tab/H3_human_ordinal.csv,
                    tab/H5_m35_ordinal.csv and tab/T10_learning.csv measure.

Data: the DIGITIZED human trade record, re-scored by informed side
(out/human_trades.csv -- 937 trades, 61 periods, 5 markets).  The published
per-period averages are in code/published_tables.py and are used only to validate
the digitization.

D is recomputed here from the design parameters and is defined ONLY where the informed side
exists (RE != vbar).  The shipped `D` column of human_trades.csv is NOT used: it is
...
ROOT = HERE.parent                       # the package root
sys.path.insert(0, str(HERE))
import ps1982_params as MK  # noqa: E402

OUT, TAB = ROOT / "out", ROOT / "tab"
FLAGGED = {(4, 1), (4, 7), (4, 8), (1, 2)}   # source_inconsistent x3 + mean_mismatch
...
def period_geometry(mnum: int, period: int, card: str | None = None,
    ...
    if card is None:
        # markets 2-5, 7-8: the card IS the state letter.  Market 1: a 10-draw sample,
...
def path_measures(prices: np.ndarray, geo: dict) -> dict:
    ...
    # per-TRADE half slopes as well: the sibling m7-8 track reports halves in per-trade
    # units, so both scalings are kept to make the two reconcilable without rescaling.
...
def human_period_table() -> pd.DataFrame:
    ...
    for (mnum, period), g in ht.groupby(["market", "period"], sort=True):
        ...
        # sanity: the digitizer's own side/RE must agree with ours
        assert geo["state"] == g["state"].iloc[0], (mnum, period)
        r = dict(geo)
        r["dot_quality_flag"] = g["dot_quality_flag"].iloc[0]
        r["count_uncertain"] = "count_uncertain" in str(r["dot_quality_flag"])
        r["flagged"] = (mnum, period) in FLAGGED
        r["mean_printed"] = g["period_mean_printed"].iloc[0]
        r["mean_digitized"] = g["period_mean_digitized"].iloc[0]
